...
public class IndexAndSqlTest {

  private static final Logger log = LoggerFactory.getLogger(IndexAndSqlTest.class);
  ...
  public void testIndexAndSQL() {
    ...
    log.debug("document: {}", docFromIndex);
  ...
  private void createUniqueIndex() {
    ...
    log.info("Create unique index for {} on {}", CLASS_NAME, Arrays.asList(PROPERTIES));
  ...
  private void save(Map<String, Object> fields) {
    log.info("Save new Document {} with {}", CLASS_NAME, fields);
    ODatabaseDocumentTx db = getDocumentTx();
    try {
  ...
  private ODocument findUniqueFromIndex(Object... values) {
    log.info("Find Document with {}", Arrays.asList(values));
    ODatabaseDocumentTx db = getDocumentTx();
    try {
  ...
  private List<ODocument> findWithSql(Object... values) {
    log.info("Find Documents with SQL {}", Arrays.asList(values));
    ODatabaseDocumentTx db = getDocumentTx();
    try {
      return db.<List<ODocument>>query(new OSQLSynchQuery<ODocument>(
  ...
  private ORecordIteratorClass<ODocument> findAll() {
    log.info("Find all documents {}", CLASS_NAME);
    ODatabaseDocumentTx db = getDocumentTx();
    try {
